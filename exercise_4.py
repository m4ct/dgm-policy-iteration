...
Alternates between:
  1. Policy evaluation  — solve the HJB PDE for the current policy via DGM,
                          updating the value network v(·,·;theta_val)
  2. Policy improvement — minimise the control-dependent Hamiltonian,
                          updating the policy network a(·,·;theta_act)

Key implementation details:
  - Value network v(·,·;theta_val) is warm-started from the previous iteration
  - Policy network a(·,·;theta_act) is frozen (requires_grad=False) during
    policy evaluation so its gradients do not interfere with the value update
  - Policy improvement uses the closed-form pointwise minimiser of the
    control-dependent Hamiltonian for fixed grad_v:
        a*(t,x) = -0.5 D^{-1} M^T grad_v(t,x)
    and trains the policy network to regress to this target

Generates:
  - plots/exercise_4_policy_iteration_loss.png
  - plots/exercise_4_value_comparison.png
  - plots/exercise_4_control_comparison.png
...
EVAL_EPOCHS = 5_000
EVAL_BATCH = 512
IMPROVE_STEPS = 3_000        # increased from 1_000 — policy needs more steps to converge
IMPROVE_BATCH = 512
LR_EVAL = 1e-3
LR_IMPROVE = 1e-3            # increased from 5e-4 — cosine schedule will decay it safely
ALPHA_INIT = np.array([1.0, 1.0])
LOG_EVERY = 500
X_RANGE = 3.0                # sample x uniformly on [-X_RANGE, X_RANGE]^d
TERMINAL_WEIGHT = 5.0        # upweight terminal boundary condition in evaluation loss
...
def policy_evaluation(value_net, policy_net, optimiser, H_t, M_t, C_t, D_t, R_t, sigma_t):
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimiser, T_max=EVAL_EPOCHS, eta_min=LR_EVAL * 1e-2
    )
    for epoch in range(1, EVAL_EPOCHS + 1):
        optimiser.zero_grad()

        # Uniform sampling on [0,T] x [-X_RANGE, X_RANGE]^d — consistent with
        # the training domain used in Exercise 3 and Exercises 2.1/2.2.
        # Gaussian sampling concentrates near the origin and starves the network
        # of gradient signal at moderate |x|, causing the value surface to be
        # poorly fitted away from zero (visible as the ~50% overestimate in the
        # PI value surface versus the analytical surface).
        t_int = torch.rand(EVAL_BATCH, 1, device=DEVICE) * T
        x_int = (2 * torch.rand(EVAL_BATCH, DIM, device=DEVICE) - 1) * X_RANGE
        res = compute_pde_residual(value_net, policy_net, t_int, x_int,
                                   H_t, M_t, C_t, D_t, sigma_t)
        pde_loss = torch.mean(res ** 2)

        # Terminal condition — uniform x, upweighted to anchor the boundary.
        x_T = (2 * torch.rand(EVAL_BATCH, DIM, device=DEVICE) - 1) * X_RANGE
...
def compute_policy_target(value_net, M_t, D_t, batch_size):
    """
    Build policy-improvement regression targets from the closed-form minimiser
    of the control-only Hamiltonian:

        argmin_a [ (grad_v)^T M a + a^T D a ]
          = -0.5 D^{-1} M^T grad_v

    grad_v is detached from the value graph so policy updates do not backprop
    into value-net parameters.
    """
    t = torch.rand(batch_size, 1, device=DEVICE) * T
    x = (2 * torch.rand(batch_size, DIM, device=DEVICE) - 1) * X_RANGE
    ...
    D_inv_t = torch.linalg.inv(D_t)
    target_a = -0.5 * (grad_v @ M_t) @ D_inv_t
    return t, x.detach(), target_a.detach()


def policy_improvement(value_net, policy_net, M_t, D_t):
    """Update policy by regressing to the closed-form Hamiltonian minimiser."""
    value_net.eval()
    opt = torch.optim.Adam(policy_net.parameters(), lr=LR_IMPROVE)
    # Cosine annealing from LR_IMPROVE to LR_IMPROVE/100 over all improvement
    # steps — gives large updates early (when the policy is far from optimal)
    # and fine-grained updates later.
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        opt, T_max=IMPROVE_STEPS, eta_min=LR_IMPROVE * 1e-2
    )

    for step in range(1, IMPROVE_STEPS + 1):
        opt.zero_grad()
        t_b, x_b, target_a = compute_policy_target(
            value_net, M_t, D_t, batch_size=IMPROVE_BATCH
        )
        a_pred = policy_net(t_b, x_b)
        loss = torch.mean((a_pred - target_a) ** 2)
        loss.backward()
        opt.step()
        scheduler.step()

        if step % 500 == 0 or step == IMPROVE_STEPS:
            print(f"    Improve step {step:5d}/{IMPROVE_STEPS}  "
                  f"MSE(target policy) = {loss.item():.4f}")
...
def main():
    ...
    for k in range(N_POLICY_ITERS):
        ...
        # 3. Monitor progress (value_net corresponds to pre-improvement policy)
        with torch.no_grad():
    ...
    # Final value evaluation for the final improved policy.
    # Without this step, value_net is stale (it solves the PDE for the
    # policy from before the last improvement), which biases final plots.
    print("\n" + "=" * 60)
    print("Final policy evaluation for plotting (post-last improvement policy)")
    print("=" * 60)
    for p in policy_net.parameters():
        p.requires_grad_(False)
    final_opt = torch.optim.Adam(value_net.parameters(), lr=LR_EVAL)
    final_losses = policy_evaluation(
        value_net, policy_net, final_opt, H_t, M_t, C_t, D_t, R_t, sigma_t
    )
    ...
    for k, losses in enumerate(all_losses):
        ...
        label = f"Iteration {k+1}" if k < N_POLICY_ITERS else "Final evaluation"
        ax.semilogy(epochs, losses, linewidth=0.6, label=label)
    ...
    fig, axes = plt.subplots(1, 3, figsize=(18.2, 6.2), subplot_kw={"projection": "3d"})
    ...
    axes[0].set_xlabel("x1"); axes[0].set_ylabel("x2"); axes[0].set_zlabel("v", labelpad=8)

    axes[1].plot_surface(X1, X2, V_pi, cmap="plasma", alpha=0.85)
    axes[1].set_title(f"Policy iteration v(t={t_val}, x)")
    axes[1].set_xlabel("x1"); axes[1].set_ylabel("x2"); axes[1].set_zlabel("v", labelpad=8)

    axes[2].plot_surface(X1, X2, np.abs(V_true - V_pi), cmap="inferno", alpha=0.85)
    axes[2].set_title(f"|Error| at t={t_val}")
    axes[2].set_xlabel("x1"); axes[2].set_ylabel("x2"); axes[2].set_zlabel("|v_true - v_PI|", labelpad=8)
    ...
    fig.tight_layout(rect=(0.0, 0.0, 1.0, 0.92), pad=1.2, w_pad=2.8)
    path = os.path.join(PLOT_DIR, "exercise_4_value_comparison.png")
    fig.savefig(path, dpi=PLOT_DPI, bbox_inches="tight")
    plt.close(fig)
    print(f"  Saved {path}")

    # ── Plot 3: Final control field vs analytical ────────────────────────
    x_range_q = np.linspace(-2.5, 2.5, 15)
    X1q, X2q = np.meshgrid(x_range_q, x_range_q)
    x_flat_q = torch.tensor(
        np.stack([X1q.ravel(), X2q.ravel()], axis=1), dtype=DTYPE, device=DEVICE
    )
    t_val_q = 0.0
    t_flat_q = torch.full((x_flat_q.shape[0],), t_val_q, device=DEVICE)
    t_flat_q2d = t_flat_q.unsqueeze(1)

    with torch.no_grad():
        a_true_q = solver.optimal_control(t_flat_q, x_flat_q).cpu().numpy()
        a_pi_q   = policy_net(t_flat_q2d, x_flat_q).cpu().numpy()

    fig, axes = plt.subplots(1, 2, figsize=(14.0, 6.4))
    for ax, a_vals, label, color in zip(
        axes,
        [a_true_q, a_pi_q],
        [f"Analytical a*(t={t_val_q}, x)", f"Policy iteration a(t={t_val_q}, x)"],
        ["steelblue", "darkorange"],
    ):
        ax.quiver(X1q.ravel(), X2q.ravel(), a_vals[:, 0], a_vals[:, 1],
                  color=color, alpha=0.85, scale=30, scale_units="inches")
        ax.set_xlabel("x1"); ax.set_ylabel("x2"); ax.set_title(label)
        ax.set_xlim(-2.8, 2.8); ax.set_ylim(-2.8, 2.8)
        ax.set_aspect("equal"); ax.grid(True, alpha=0.3)

    fig.suptitle(
        f"Exercise 4 — Analytical vs policy-iteration control at t={t_val_q}",
        fontsize=13,
    )
    fig.tight_layout(rect=(0.0, 0.0, 1.0, 0.94), pad=1.1, w_pad=2.2)
    path = os.path.join(PLOT_DIR, "exercise_4_control_comparison.png")
    fig.savefig(path, dpi=PLOT_DPI, bbox_inches="tight")
    plt.close(fig)
    print(f"  Saved {path}")
