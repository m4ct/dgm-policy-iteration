...
def main():
    ...
    fig, axes = plt.subplots(2, 2, figsize=(13.8, 11.6))
    ...
    fig.tight_layout(rect=(0.0, 0.0, 1.0, 0.95), pad=1.1, w_pad=2.0, h_pad=2.0)
    ...
    ax1.set_ylim(0.0, max(mse_vals) * 1.12)
    for i, v in enumerate(mse_vals):
        ax1.text(i, v * 1.03, f"{v:.2e}", ha="center", fontsize=10)
    ...
    ax2.set_ylim(0.0, max(rel_vals) * 1.12)
    for i, v in enumerate(rel_vals):
        ax2.text(i, v * 1.03, f"{v:.2e}", ha="center", fontsize=10)

    fig.suptitle("Exercise 2.2 - component-wise control error", fontsize=13)
    fig.text(
        0.5,
        0.92,
        "a2's larger absolute MSE is mostly a scale effect.\n"
        "Relative MSE shows the two control components are learned similarly.",
        ha="center",
        va="top",
        fontsize=10,
    )
    fig.tight_layout(rect=(0.0, 0.0, 1.0, 0.84), pad=1.1, w_pad=2.0)
